...
struct
  ...
  let rec eval mem env e =
    ...
    | IF (ec, et, ef) ->
        ...
        let b = value_bool vc in
        let er = if b then et else ef in
        eval mem' env er
    | WHILE (ec, er) ->
        let (vc, mem') = eval mem env ec in
        let b = value_bool vc in
        if b then (
          let (_, mem'') = eval mem' env er in
          eval mem'' env (WHILE (ec, er))
        )
        else (Unit, mem')
    | LETV (x, e1, e2) ->
    ...
    | LETF (x, xs, e1, e2) ->
        eval mem (Env.bind env x (Proc (xs, e1, env))) e2
    | CALLV (f, exp_list) ->
        let (id_list, fexp, fenv) = lookup_env_proc env f in
        let rec caller exps ids mem fenv =
          match (exps, ids) with
          | ([], []) -> eval mem (Env.bind fenv f (Proc (id_list, fexp, fenv))) fexp
          | (e::exps', i::ids') ->
              let (v, mem') = eval mem env e in
              let (l, mem'') = Mem.alloc mem' in
              caller exps' ids' (Mem.store mem'' l v) (Env.bind fenv i (Addr l))
          | _ -> raise (Error "InvalidArg")
          in
          caller exp_list id_list mem fenv
    | CALLR (f, id_list) ->
        let (rid_list, fexp, fenv) = lookup_env_proc env f in
        let rec caller ids rids mem fenv =
          match (ids, rids) with
          | ([], []) -> eval mem (Env.bind fenv f (Proc (id_list, fexp, fenv))) fexp
          | (x::ids', rx::rids') ->
              let l = lookup_env_loc env x in
              caller ids' rids' mem (Env.bind fenv rx (Addr l))
          | _ -> raise (Error "InvalidArg")
          in
          caller id_list rid_list mem fenv
    | RECORD rs ->
        ( match rs with
        | [] -> (Unit, mem)
        | _ -> (
            let rec mapper mem rs =
              match rs with
              | [] -> ((fun _ -> raise (Error "Unbound")), mem)
              | (x, e)::rs' ->
                  let (v, mem') = eval mem env e in
                  let (l, mem'') = Mem.alloc mem' in
                  let (r, mem''') = mapper (Mem.store mem'' l v) rs' in
                  ((fun x' -> if x' = x then l else r x'), mem''')
            in
            let (r, mem') = mapper mem rs in
            (Record r, mem')
          )
        )
    | FIELD (e, x) ->
