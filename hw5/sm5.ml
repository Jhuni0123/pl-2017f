  ...
  let malloc_with_gc s m e c k =
    if List.length m < mem_limit then
      ...
      and mark_value v =
        match v with
        | L l -> mark_loc l
        | R r -> List.iter (fun (_, l) -> mark_loc l) r
        | _ -> ()
      and mark_command com =
        List.iter mark_cmd com
      and mark_cmd c =
        match c with
        | PUSH o -> mark_obj o
        | JTR (com1, com2) -> mark_command com1; mark_command com2
        | _ -> ()
      and mark_obj o =
        match o with
        | Val v -> mark_value v
        | Fn (_, com) -> mark_command com
        | _ -> ()
      and mark_env e =
        List.iter (fun (_, ev) -> mark_evalue ev) e
      and mark_evalue ev =
        match ev with
        | Loc l -> mark_loc l
        | Proc (_, com, e) -> mark_command com; mark_env e
      and mark_svalue sv =
        match sv with
        | V v -> mark_value v
        | P (_, com, e) -> mark_command com; mark_env e
        | M (_, ev) -> mark_evalue ev
      in
      let _ = List.iter mark_svalue s in
      let _ = mark_env e in
      let _ = List.iter (fun (com, e) -> mark_command com; mark_env e) k in
